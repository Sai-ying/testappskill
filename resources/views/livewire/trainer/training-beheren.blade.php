<div>
    <h1 class="m-5 font-bold">Trainingen Beheren</h1>
    {{-- Filters --}}
    <x-tmk.section class="mb-4 flex gap-2">
        ...
        {{--<x-tmk.form.switch id="showPast" wire:model.live="showPast"
                           ...
                           class="w-20 h-auto"/>--}}
        <x-tmk.form.button color="nieuw" wire:click="newTraining()">Nieuwe training</x-tmk.form.button>
    </x-tmk.section>
                    ...
                    <td>
                        <div class="border border-gray-300 rounded-md overflow-hidden m-2 grid grid-cols-2 h-10">
                            <button class="text-gray-400 hover:text-sky-100 hover:bg-sky-500 transition border-r border-gray-300"
                                    wire:click="editTraining({{ $item->id }})">
                                <x-phosphor-pencil-line-duotone class="inline-block w-5 h-5"/>
                            </button>
                            <button
                                class="text-gray-400 hover:text-red-100 hover:bg-red-500 transition"
                                wire:click="deleteTraining({{ $item->id }})"
    ...
    <x-dialog-modal wire:model.live="showModal">
        <x-slot name="title">
            <h2>{{ is_null($form->id) ? 'Nieuwe training' : 'Edit training' }}</h2></x-slot>
        <x-slot name="content">
            @if ($errors->any())
            ...
            <x-input  id="address" type="text" placeholder="address"
                ...
                @if(is_null($form->id))
                    <div class="flex">
                        ...
                        <div class="w-1/2 ml-2">
                            <x-label value="Herhaal voor hoeveel werken"/>
                            <x-input id="weeks" type="number" min="1" placeholder="5"
                                     wire:model="form.weeks"
                                     class="mt-1 block w-full"/>
                        </div>
                    </div>
                @else
